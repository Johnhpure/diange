  ...
  >
    <Transition name="fade" mode="out-in">
      <div v-if="songData">
        ...
        <n-card
          v-if="!selectedSource.includes('unblock') && !selectedSource.includes('meting')"
          class="set-item"
        >
          <div class="name">
            音质选择
            <n-tag
              v-if="qualityOptions.find((q) => q.value === selectedQuality)?.lossless"
              :bordered="false"
              round
              size="small"
              type="success"
        ...
        <n-card v-if="selectedSource === 'unblock'" class="set-item">
          ...
          <n-checkbox-group v-model:value="musicSourceChecked" style="margin-bottom: 8px">
            <n-checkbox v-for="item in musicSourceOptions" :key="item.value" :value="item.value">{{
              item.label
            }}</n-checkbox>
          </n-checkbox-group>
...
import {
  getSongDetail,
  getSongLyricLegacy,
  getSongDownloadFromPyncmd,
  getSongDownload,
  getMusicNumUrlNew,
  getMetingSongDownload,
  getPythonSongDownload,
} from "@/api/song";
...
// 复选框选中项
const musicSourceChecked = ref(
  settings.customMusicSource
    ?.split(",")
    .map((i) => i.trim())
    .filter(Boolean) || [],
);
...
const selectedSource = ref("pyncmd");
