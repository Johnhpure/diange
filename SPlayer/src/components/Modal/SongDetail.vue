<template>
  ...
  >
    ...
    <div v-if="songDetail" class="detail">
      <div class="pic">
        <n-image
          class="coverImg"
      ...
      <div class="right">
        <div class="intr">
          ...
          <n-text v-if="songDetail.alia" depth="3" class="alia">{{ songDetail.alia }}</n-text>
          <n-space class="tag">
            <n-tag
              v-if="songDetail.fee == 1 || songDetail.fee == 4"
              class="vip"
              round
              :bordered="false"
            >
          ...
          <div v-if="songDetail.album" class="item">
            <n-icon :depth="3">
              <SvgIcon icon="disc" />
            </n-icon>
        ...
        <n-space class="button">
          ...
          <n-button
            ...
            strong
            secondary
            @click="router.push(`/videos-player?id=${songDetail.mv}`)"
          >
            <template #icon>
              <n-icon>
                <SvgIcon icon="video" />
    ...
    <!-- 热门评论 -->
    <div v-if="hotCommentData.length > 0" class="comments">
    ...
    <!-- 相似歌单 -->
    <div v-if="simiPlayList.length > 0" class="simiPlayList">
      <n-divider />
...
const getSongDetailData = async (ids) => {
  ...
  try {
    ...
    $message.error(
      error.message === "未找到歌曲信息" ? error.message : "获取歌曲详情失败，请稍后重试",
    );
  }
};
