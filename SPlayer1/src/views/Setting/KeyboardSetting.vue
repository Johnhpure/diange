  ...
  <div class="keyboard-setting set-type">
    ...
    <n-collapse>
      <n-collapse-item name="shortcut" arrow-placement="right">
        <template #header>
          <div class="collapse-title">快捷键设置</div>
        </template>
        ...
        >
          <div v-for="(item, key, index) in shortcutListRef" :key="index" class="shortcut-row">
...
const inputBlur = () => {
  ...
  if (selectGlobal.value && shortcutStore.globalOpen && isElectron) {
    ...
    if (currentShortcut && currentShortcut.shortcut) {
      message.success(`已更新本地快捷键: ${currentShortcut.name}(${currentShortcut.shortcut})`, {
        duration: 3000,
      });
    } else {
