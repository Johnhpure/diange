...
const annContene =
  import.meta.env.RENDERER_VITE_ANN_CONTENT ||
  "本站资源限制可能随时超出, 推荐在Github页面下载客户端或者自部署使用; 网站资源均来自第三方, 仅供学习交流使用, 请勿用于商业用途";
  ...
  try {
    ...
    if (webFonts && lyricsFont) {
      document.documentElement.style.setProperty(
        "--main-font-family",
        `"${webFonts}", system-ui, -apple-system, sans-serif`,
      );
      document.documentElement.style.setProperty(
        "--main-font-family-lyric",
        `"${lyricsFont}", system-ui, -apple-system, sans-serif`,
      );
    }
